...
import '../detail_meeting_user.dart';

class JadwalMeeting extends StatefulWidget {
  @override
  _JadwalMeetingState createState() => _JadwalMeetingState();
}

class _JadwalMeetingState extends State<JadwalMeeting> {
                            ...
                            onTap: () {
                              Navigator.push(context,
                                  MaterialPageRoute(builder: (context) =>
                                      DetailMeetingUser(
                                          dataMeeting: listDataMeeting[index],
                                          isRegistered: checkBool(listDataMeeting[index])))).then((value){
                                            if(value!=false){
                            ...
                            child: Container(
                              ...
                              child: Card(
                                  ...
                                  child: ListTile(
                                      ...
                                      subtitle: Text(
                                        listDataMeeting[index]['time'],
                                        style: TextStyle(
                                            color: Color(0xffadaad6)),),
                                      trailing: Text(register(listDataMeeting[index]),style: TextStyle(color: Colors.white, fontSize: 10),)
                                  )
                              ),
                            )
                        ),
  ...
  String register(dynamic dataMeeting){
    for (int i = 0; i < dataMeeting['users'].length; i++) {
      if (dataMeeting['users'][i]['id'] == idUser) {
         return 'Registered';
      }
      else {
          return 'Unregistered';
  ...
  bool checkBool(dynamic dataMeeting) {
    for (int i = 0; i < dataMeeting['users'].length; i++) {
      if (dataMeeting['users'][i]['id'] == idUser) {
            return true;
      }
